...
res <- 0.1 #desired resolution in degrees. This cannot be changed in this version.
...
subfolder <- 'Data' #name of subfolder. If not in one, leave as is.
eGRID <- FALSE #If true will automatically convert eGRID data in the format we have it to the format it needs to be in. 
...
filename <- 'RIeGRID.csv' #The file that contains your data
...
for (i in 1:nStates)
{
    ...
    if (!coordsNA){
      if (is.na(actualCountyIndex)) 
      {
        ...
        } else if (locTest$STATE != stateName)
        {
          ...
          if (is.na(nameTest)) errStr <- 'bigger'
  ...
  summedv <- aggregate(tmp$v,by=list(tmp$x,tmp$y),FUN=sum)
